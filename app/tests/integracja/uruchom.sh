...
for nazwa in test_4a test_4b test_4c test_7 test_indeks test_kluby_templaty \
             test_mapowania test_powiadomienia test_remember test_xg; do
...
if [ "$BEZ_HTTP" -eq 1 ]; then
  for nazwa in test_sesja_http test_haslo_http test_klub_hub_http test_mapowania_http; do
    pomin "$nazwa" "--bez-http"
  done
else
  # KOLEJNO, NIGDY RÓWNOLEGLE: każdy zestaw podnosi wbudowany serwer PHP na
  # stałym porcie (8946, 8947, 8951+8952, 8961). Dwa naraz biłyby się o port,
  ...
  # Ten jeden potrzebuje jeszcze silnika Pythona.
  if [ -x "$PYTHON" ]; then
    zestaw "test_mapowania_http" env PYTHONPATH="$KORZEN/engine" php "$TUTAJ/test_mapowania_http.php"
  else
